...
contract NutsPlatform is FspRole, TimerOracleRole {
    ...
    function createIssuance(address instrumentAddress, string memory sellerParameters) public returns (uint256) {
        ...
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.createIssuance(issuanceId,
    ...
    function engageIssuance(uint256 issuanceId, string memory buyerParameters) public {
        ...
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.engage(issuanceId,
    ...
    function deposit(uint256 issuanceId, uint256 amount) public {
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processDeposit(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
    ...
    function depositToken(uint256 issuanceId, address tokenAddress, uint256 amount) public {
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processTokenDeposit(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
    ...
    function withdraw(uint256 issuanceId, uint256 amount) public {
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processWithdraw(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
            balances, msg.sender, amount);
        // Revoke writer role afterward
        issuanceStorage.removeWriter(commonProperties.instrumentAddress);
    ...
    function withdrawToken(uint256 issuanceId, address tokenAddress, uint256 amount) public {
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processTokenWithdraw(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
            balances, msg.sender, tokenAddress, amount);
        // Revoke writer role afterward
        issuanceStorage.removeWriter(commonProperties.instrumentAddress);
    ...
    function processScheduledEvent(uint256 issuanceId, uint256 timestamp, string memory eventName,
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processScheduledEvent(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
    ...
    function processCustomEvent(uint256 issuanceId, string memory eventName, string memory eventPayload) public {
        ...
        // Grant a temporary writer role to the instrument.
        issuanceStorage.addWriter(commonProperties.instrumentAddress);
        (Instrument.IssuanceStates updatedState, string memory transfers) = instrument.processCustomEvent(issuanceId,
            Instrument.IssuanceStates(commonProperties.state), issuanceStorage,
            balances, eventName, eventPayload);
        // Revoke writer role afterward
        issuanceStorage.removeWriter(commonProperties.instrumentAddress);
    ...
    function getIssuanceProperties(uint256 issuanceId) private view returns (CommonProperties.Data memory commonProperties) {
        ...
        string memory commonPropertiesData = _storage.getString(getIssuanceCommonDataKey(issuanceId));
    ...
    function saveIssuanceProperties(uint256 issuanceId, CommonProperties.Data memory commonProperties,
        ...
        _storage.setString(getIssuanceCommonDataKey(issuanceId), string(commonProperties.encode()));
    }
