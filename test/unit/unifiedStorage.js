...
contract('UnifiedStorage', (accounts) => {
    ...
    it('should add a new key-value pair of strings', async () => {
       await contractInstance.setValue('this is new key', 'this is new value');
       const valueContent = await contractInstance.getValue('this is new key');
       
       assert.equal(valueContent, 'this is new value', 'The content of the value is not correct');
    }),
    it('should update the value', async () => {
      await contractInstance.setValue('this is old key', 'this is old value');
      await contractInstance.setValue('this is old key', 'this is new value');
      const valueContent = await contractInstance.getValue('this is new key');
      
      assert.equal(valueContent, 'this is new value', 'The content of the value is not correct');
   }),
   it('should return empty value', async () => {
      const valueContent = await contractInstance.getValue('this is another   key');
